...
def __(
    ...
    if train_button.value > 0:
        ...
        # Calculate metrics
        mae = np.mean(np.abs(all_predictions - y_data))
        rmse = np.sqrt(np.mean((all_predictions - y_data) ** 2))
...
def __(PLOTLY_AVAILABLE, go, mo, training_results):
    if training_results is not None and PLOTLY_AVAILABLE:
        ...
        fig.add_trace(go.Scatter(
            x=epochs_range, y=training_results['train_losses'], 
            name='Train Loss', line=dict(color='blue')
        ))
        fig.add_trace(go.Scatter(
            x=epochs_range, y=training_results['val_losses'], 
...
def __(
    ...
    if training_results is not None and all(x is not None for x in [pred_target, pred_days, pred_vol, pred_rsi]):
        ...
        with torch.no_grad():
            ...
            prediction = training_results['model'](torch.FloatTensor(live_features_scaled))
...
@app.cell
def __(PLOTLY_AVAILABLE, go, mo, np, training_results):
    if training_results is not None and PLOTLY_AVAILABLE:
        # Performance analysis
        predictions = training_results['predictions']
        actuals = y_data  # y_data from earlier cell
